...
class Command : CommandExecutor {
	override fun onCommand(sender: CommandSender, p1: Command, p2: String, p3: Array<out String>): Boolean {
		if (sender is Player) {
			...
			val profile = GameProfile(UUID.randomUUID(), "Bernard")
			...
			player.connection = ServerGamePacketListenerImpl(server, Connection(PacketFlow.CLIENTBOUND), player)
			player.setPos(location.x, location.y, location.z)
			player.bukkitEntity.noDamageTicks = 0

			Bukkit.getOnlinePlayers().forEach {
				val packet = ClientboundPlayerInfoPacket(ClientboundPlayerInfoPacket.Action.ADD_PLAYER, player)
